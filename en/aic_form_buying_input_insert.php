...
// 24 juillet 2009 
	...
	*/
	...
	$qty1=$_POST['tbuying_qty_pack2'] * $_POST['tbuying_pack_size'];
	$QTY_TEMP1=$_POST['tbuying_qty_pack2'] * $_POST['tbuying_pack_size'];
	$qty1=sprintf("%.4f",$qty1);
	$QTY_TEMP1=sprintf("%.4f",$QTY_TEMP1);
	/*
...
// $PRICE_PER_PACK_TEMP=sprintf("%.4f",$_POST['tbuying_price_per_pack']);
	...
	if ( $_POST['tbuying_ppu'] <= 0)
		$PPU=$_POST['tbuying_price_per_pack'] / $_POST['tbuying_qty_pack'];
		else 
		$PPU=sprintf("%.4f",$_POST['tbuying_ppu']);
 echo "PPUT=$PPU <BR>";
 */
 if (isset($_POST['tbuying_ppu'])) {
	...
	$ppu1=$_POST['tbuying_price_per_pack'] / $_POST['tbuying_qty_pack'];
	$PPU=$_POST['tbuying_price_per_pack'] / $_POST['tbuying_qty_pack'];
	$ppu1=sprintf("%.4f",$ppu1);
	$PPU=sprintf("%.4f",$PPU);
	 /*
...
// $PRICE_PER_SMALLEST_UNIT=$TOTAL_COST/ $TOTAL_QTY;
	...
	// echo "PPP_EUR=$PPP_EUR, PPU_EUR=$PPU_EUR,  PPU_DOL=$PPU_DOL, PPP_DOL=$PPP_DOL <BR>";
...
*/
  ...
  // echo " Ligne a inserer=$insertSQL <BR>";
  	...
  	mysqli_select_db($cibproto, $database_cibproto);
// $Result1 = mysqli_query($insertSQL, $cibproto) or die(mysqli_error($cibproto))

	  if (!$Result1= mysqli_query($cibproto, $insertSQL)) {
 		 $return = new Exception("Erreur SQL");
		$tbl = $return->getTrace();
	// echo " Insert=$insertSQL <BR>";
		$errorno=mysqli_errno($cibproto);
		$errorsql=mysqli_error($cibproto);
...
$Re_tbuying = mysqli_query($cibproto, $query_Re_tbuying) or die(mysqli_error($cibproto));
...
WHERE tproduct_status = '1' AND  (tproduct_cat = '$pCate') order by tproduct_desc";

}
else{
$query_Re_prod = "SELECT * FROM $Product WHERE tproduct_status = '1' and (tproduct_desc not like '%***%') order by tproduct_id ";

}
 

$Re_product = mysqli_query($cibproto, $query_Re_prod) or die(mysqli_error($cibproto));
...
$Re_dosage = mysqli_query($cibproto, $query_Re_dosage) or die(mysqli_error($cibproto));
...
$Re_dosage1 = mysqli_query($cibproto, $query_Re_dosage1) or die(mysqli_error($cibproto));
