...
<html>
<head>
<title>List of product  by price for each supplier - aic_list_supp_price_prod.php</title>
...
<table width="963" border="1" cellpadding="2" cellspacing="2">
  <tr>
    <th width="126" scope="row"><div align="center" class="Style25"><a href="<?php echo $pForm_Name ?>" title="Back" class="Style7 Style6 "><span class="Style88">Back</span></a></div></th>
  ...
  <?php do {
   ?>
    ...
    <?php 
	 ...
	 $compteur=0;
	//  echo "RRRRXXXXXAIMP row_Regroupsup.tbuying_prov_id=$AIMP<BR>";
	 mysqli_select_db($database_cibproto, $cibproto);
...
FROM $view_table_name where tprov_id=$AIMP and tbuying_status='1'";
	 ...
	 $query_Redetsup.= " AND ({$tbuying_ppu} >= '$pMini' and {$tbuying_ppu} <= '$pMaxi'  or ('$pMini' is NULL and '$pMaxi' is NULL))
 ...
 */
	...
	// echo "totalRows_Redetsup=$totalRows_Redetsup <BR>";
	 ...
	 <th width="2%" nowrap scope="col"><span class="Style30 Style60"> Unit price(smallest unit)</span></th> 
	  <th width="2%" nowrap scope="col"><span class="Style30 Style60"> Quantity (smallest unit)</span></th> 
      ...
      <th width="5%" scope="col"><span class="Style30">Transportation method</span></th>
	    ...
	    <th width="5%" scope="col"><span class="Style9 Style30">Comments</span></th>
             ...
             <td nowrap><div align="center"><span class="Style63"><?php if ( $CUR=='$') {echo $CUR.number_format($row_Redetsup[$tbuying_price_per_pack],4);}
	                                                                       else {echo number_format($row_Redetsup[$tbuying_price_per_pack],4).$CUR;}
			                                                       ?></div></span></td>
              <td nowrap><div align="center"><span class="Style63"><?php echo number_format($row_Redetsup['tbuying_qty_pack']); ?></div></span></td>
  <td nowrap><div align="center"><span class="Style63"><?php   if ( $CUR=='$') {echo $CUR.number_format($row_Redetsup[$tbuying_tcost],4);}
	                                                                       else {echo number_format($row_Redetsup[$tbuying_tcost],4).$CUR;}
															      ?></div></span></td>
	          <td nowrap><div align="center"><span class="Style63"><?php echo $row_Redetsup['tinco_desc']; ?></div></span></td>
              <td nowrap><div align="center"><span class="Style63"><?php if ( $CUR=='$') {echo $CUR.number_format($row_Redetsup[$tbuying_ppu],4);}
	                                                                       else {echo number_format($row_Redetsup[$tbuying_ppu],4).$CUR;}
			                                                       ?></div></span></td>
			  <td nowrap><div align="center"><span class="Style63"><?php echo number_format($row_Redetsup['tbuying_qu']); ?></div></span></td>
