...
//$pMaxi = $_POST['pMaxi'] ;
//$pCure = $_POST['pCure'] ;
if (isset($_POST['pDsta'])){  
	$pDsta = $_POST['pDsta'] ;
}
else echo "Data Start pose probleme <br>";	

if (isset($_POST['pDend'])){  
	$pDend = $_POST['pDend'] ;
}
else echo "Data End pose probleme <br>"; 
/*
$pDsta = $_POST['pDsta'] ;
$pDend = $_POST['pDend'] ;
*/
...
and tbuying_status = '2' ";
$query_RePriceLow.= $param_cat.$param_prod.$param_prov;
$query_RePriceLow.= " AND (tbuying_dbid >= STR_TO_DATE('$pDsta','%d-%m-%Y') 
	and (tbuying_dbid <= STR_TO_DATE('$pDend','%d-%m-%Y') ) )
	ORDER By DATE_FORMAT(tbuying_dbid,'%d-%m-%Y') desc, tproduct_cat, tproduct_desc ";
/* chnaged on MArch 4th 2011 in Freetown
$query_RePriceLow.= " AND (tbuying_ppu between '$pMini' and '$pMaxi' or ('$pMini' is NULL and '$pMaxi' is NULL))
AND (tbuying_dbid >= STR_TO_DATE('$pDsta','%d-%m-%Y') 
	and (tbuying_dbid <= STR_TO_DATE('$pDend','%d-%m-%Y') ) )
	ORDER By DATE_FORMAT(tbuying_dbid,'%d-%m-%Y') desc, tproduct_cat, tproduct_desc ";
...
$RePriceLow = mysqli_query($cibproto, $query_limit_RePriceLow) or die(mysqli_error($cibproto));
...
if (isset($_GET['totalRows_RePriceLow'])) {
  ...
  $all_RePriceLow = mysqli_query($cibproto, $query_RePriceLow);
...
if (!mysqli_select_db($cibproto, $database_cibproto))
{ 
...
<table width="100%"  border="1" cellspacing="2" cellpadding="2" summary="Lowest Price per period">
  ...
  <tr>
    ...
    <th width="5%" scope="col"><span class="Style30 Style9"><strong>Packaging type</strong></span></th>
    ...
    <th width="6%" nowrap scope="col"><span class="Style30"><strong>Date of bid opening / Date of reception </strong></span></th>
...
<div align="center"><span class="Style25"><span class="Style88"><a href="export_excel_report.php?requete=<?PHP echo $query_RePriceLow ?>&choix=0 &cur='$'" class="Style6 Style25 Style88"><span class="Style62 Style25"><span class="Style86">Export to</span></span> <span class="Style93">excel</span></a></span></span></div>
</body>
</html>
<?php
